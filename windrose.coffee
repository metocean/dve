class ERwindrose
  constructor: (svg, data, dimensions, options, domain) ->
    ...
    @inner = @svg
      .append 'g'
      .attr 'class', 'inner'
      .attr 'transform', "translate(#{@canvas.left + @canvas.width/2},#{@canvas.top + @canvas.height/2})"
    ...
    @axis
      ...
      .attr 'y1', @scale 0
      .attr 'x2', @scale 0
      .attr 'y2', @canvas.width/2

    @axis
      .append 'g'
      .attr 'transform', (d) => "translate(#{@scale 0},#{(@canvas.height * (-0.53))})"
      .append 'text'
      .attr 'transform', (d) => "rotate(#{-d.key})"
